char *dialv = "Dial Command, 4E+(010) 27 Sep 87";
 ...
 * Modifications:
 *
 *	26-Sep-87	Added support for 'builtin' modem types, with one
 *			instance - the UNIX PC internal modem.
 *
...
static
MDMINF BILTIN =	/* Info for modem controlled by IOCTL's */
		/* Most of these things are meaningless in the context of
			modems controlled thru the device driver.  Rather
			than to ambiguate their meanings by using them for
			other things, I just set them to NULL or zero. */
		/* This structure will compile in even in systems without
			a builtin modem.  This is so that the numbering
			scheme still works, and the ptrtab[] array has
			something to point to. */
    {
#ifdef UNIXPC
    40,			/* dial-time for UNIX PC */
    ",:",		/* pause-chars */
    2,			/* 2-second pause (actually, : causes 10 secs) */
#else
    0,			/* dial_time */
    NULL,		/* pause_chars */
    0,			/* pause_time */
 ...
 */
/*
 * The "internal" entry exists only in systems where it has meaning,
 * so that "?" doesn't show it, and you cannot totally befuddle kermit
 * by selecting a non-existent set of features.
 */
...
#ifdef INTMDM
#ifdef UNIXPC
	if (strncmp(ttname, "/dev/ph", sizeof("/dev/ph")-1) != 0
		&& mdmtyp != n_BILTIN) {
	    printf("Sorry, the phone lines require you to use the command\
'set modem-dialer internal'\n");
	    return(-2);
	}
#endif
	if (mdmtyp == n_BILTIN) {
#ifdef UNIXPC
	    if (strncmp(ttname, "/dev/ph", sizeof("/dev/ph")-1) != 0) {
		printf("Sorry, you must 'set line' to a phone line to use the\n\
internal modem.\n");
		return(-2);
	    }
#else
	    printf("Sorry, there has been a configuration error with the\n\
internal modem.\n");
#endif
	}
	if (mdmtyp == n_BILTIN) {
 ...
 */
	...
	debug(F110,"Failure jump in dial.c, type ",F_reason[n],0);
...
/* ...dial, cont'd */
	...
	if (ttgetp(&updata) != 0) {
	    ...
	    (void) tthang();
	    ttpkt(speed, CONNECT, parity);
	    reset();
	    return(-2);
	}
	/* check the (read-only) mode bit */
	if ((updata.c_lineparam & (VOICE | DATA)) != DATA) {
	    printf("Sorry, can't open the line for a data call.\n");
	    (void) tthang();
	    ttpkt(speed, CONNECT, parity);
	    reset();
	    return(-2);
	}
	updata.c_lineparam &= ~INCMNG;
	updata.c_feedback  = SPEAKERON | NORMSPK | NORMRNG | MEDRNG ;
	if (ttsetp(&updata) != 0) {
	    printf("Sorry, can't set modem parameters.\n");
	    (void) tthang();
	    ttpkt(speed, CONNECT, parity);
	    reset();
	    return(-2);
	...
	if (ttpkup(&updata) != 0) {
	    printf("Sorry, can't go offhook -- is the line plugged in?\n");
	    (void) tthang();
	    ttpkt(speed, CONNECT, parity);
	    reset();
	    return(-2);
 ...
 */
	...
	case n_BILTIN:
		...
		strcpy(lbuf, "Ioctl failure");
		status = FAILED;
		break;
	    }
	    if (updata.c_linestatus & MODEMCONNECTED) {
		/* finally, go into delay-mode */
		debug(F110," Modem connected",ttname,0);
		if (ttdmode(ttname)) {
		    printf("Sorry, can't reset file parameters.\n");
		    strcpy(lbuf, "Fcntl failure");
		    status = FAILED;
		    break;
		}
		debug(F110," Line now will block",ttname,0);
