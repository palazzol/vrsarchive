char *ckxv = "Unix tty I/O, 4E+(047), 27 Sep 87";
...
#ifdef UNIXPC
#include <sys/phone.h>			/* structures for internal modem */
#endif
...
#else
#ifdef UNIXPC
/* AT&T UNIX PC (7300 and 3b1) System V support by */
/* Kevin O'Gorman, Oxnard, CA, kevin@kosman */
char *ckxsys = " AT&T UNIX PC System V";
#else
...
tthang() {
    ...
    debug(F110,"Hanging up line",ttnmsv,0);
#ifdef aegis
...
#else  /* not HPUX */
#ifdef UNIXPC
    if (strncmp( ttnmsv, "/dev/ph", sizeof("/dev/ph")-1) == 0) {
	int res;
	extern int ttdisc();
	ttdisc();  /* hang up the phone */
    } else {
#endif
...
#ifdef UNIXPC
/* If this is one of the UNIX PC internal lines, we MUST reopen with
 * O_NDELAY, or else the system thinks we want to wait for an incoming
 * call.
 */
    if (strncmp(ttnmsv, "/dev/ph", sizeof("/dev/ph")-1) == 0) {
	ttc_save |= O_NDELAY;
...
ttres() {				/* Restore the tty to normal. */
    ...
    debug(F111,"Ttres, restoring terminal",ttnmsv,ttyfd);
...
/*	Getty-get-off function.  This function gets the 'getty(1m)' off
	the indicated line.  The return codes seen are:
...
*/
static int Offreslt;
static char savttnam[10];

static int
Getoffsh(path) register char *path;
{	char Systmcmd[33];
	while (*path != '\0') path++;
	path -= 3;
	strcpy(savttnam,path);
	sprintf(Systmcmd,"/usr/bin/getoff.sh %s",path);
	debug(F110,"System command ",Systmcmd,0);
	return system(Systmcmd);
}

static void
Getonshl(path) register char *path;
{	char Systmcmd[33];
	while (*path != '\0') path++;
	path -= 3;
	sprintf(Systmcmd,"/usr/bin/geton.sh %s",path);
	debug(F110,"System command ",Systmcmd,0);
	system(Systmcmd);
}
...
/* T T G E T P */
int
ttgetp(updata) struct updata *updata;
{
	return ioctl(ttyfd, PIOCGETP, updata);
}

/* T T S E T P */
int
ttsetp(updata) struct updata *updata;
{
	return ioctl(ttyfd, PIOCSETP, updata);
}
...
ttdisc(updata)
{
	return ioctl(ttyfd, PIOCDISC);
}

/* T T P K U P */
int
ttpkup(updata) struct updata *updata;
{
	return ioctl(ttyfd, PIOCOFFHOOK, updata);
}

/* T T D I A L */
int
ttdial(digit) char digit;
{
	return ioctl(ttyfd, PIOCDIAL, &digit);
}

/* T T D M O D E */
int
ttdmode(ttname) char *ttname;
{
	struct updata u;
	int s;
	debug(F101,"   Setting line blockable","",ttyfd);
	s = fcntl(ttyfd, F_GETFL);
	debug(F111,"   File status",ttname,s);
	if (fcntl(ttyfd, F_SETFL, fcntl(ttyfd, F_GETFL, 0) & ~O_NDELAY)
		< 0) {
	    return(1);
	}
	s = fcntl(ttyfd, F_GETFL);
	debug(F111,"   File status",ttname,s);
	ttgetp(&u);
	debug(F101,"   Status","",u.c_linestatus);
	debug(F110,"   Resetting mode",ttname,0);
	close(open(ttname, O_RDWR)); /* reopen the line */
	debug(F100,"   Line reset","",0);
	return (0);
}
...
ttlock(ttfd) char *ttfd; {		/* lock uucp if possible */
    ...
    close (lck_fil);
#ifdef UNIXPC
    /* If there was no lock file, nobody was using the line, but there may
     * have been a getty (or uugetty) on it.  This has to be thrown off, or
     * we'll run into trouble later with competition for incoming characters.
     * Use the User Agent script in /usr/bin for this purpose, and if it fails,
     * remove our own lock file and return an error status.  For now, this
     * case causes a confusing error message about lock files, but it's one
     * of those "never happen" cases anyway.
     */
    if ((Offreslt = Getoffsh(ttfd)) <= 0) {
	unlink(flfnam);
	return -1;
    }
#endif /*UNIX PC*/
...
ttunlck() {				/* kill uucp lock if possible */
    int savres = 0;
    debug(F110,"Unlocking uucp",flfnam,0);
...
/* Try to make reads nonblocking */
    ...
    if (flow == DIALING
#ifdef UNIXPC
		/* don't do this yet if using the ioctl dialer */
...
/*** It's not really clear to me how to determine if we're running in the ***/
/*** background, or what the definition of background should be.  Anyone  ***/
/*** who has a clearer idea, please pass it along.  - Frank               ***/
...
#ifdef SIGTSTP
	signal(SIGTSTP,f);		/* Keyboard stop */
#endif
#ifdef UXIII
