...
struct rinst *curproc, *initfree(), *newproc(), *add2call(), *ok;
...
*/
	for (i = 1 ; i<argc && argv[i][0]=='-'; i++){
		if (match(argv[i],"-c")) ;
			...
			terse=0;
		else if (match(argv[i],"-changes")) (void) printf("-changes not yet implemented\n");
		else if (match(argv[i],"-width") || match(argv[i],"-w")){
			ntabs = (atoi(argv[++i])-20)/8;
		}
		else (void) printf ("unknown command %s", argv[i]);
		(void) printf ("\n");
		}
	ok = (struct rinst *)1;
	curproc = (struct rinst *)0;
	while ((gf = getfunc(atom)) != -1 && ok ){
		if (gf) ok = add2call(atom,curproc);
		else ok = curproc = newproc(atom);
	}
	if (i<argc){
		do{
			startfunc = argv[i];
			...
			else (void) printf ("*** error *** %s not found\n",startfunc);
		}while (++i<argc);
	}
	else {
	...
	for(;;){
		...
		if (isalpha(c)){
			...
			case '(':
				...
				if (!checksys(atom)) {
					if (!bracket) return (0);
					if ((ss=seen(atom))==-1)(void) printf("aseen[] overflow\n");
...
struct rinst *ret;
	ret=frp;
	if (!ret) (void) printf ("out of instance blocks\n");
	frp=frp->calls;
	return(ret);
}
...
int i, tabd, tabstar, tflag;
	...
	++linect;
	(void) printf ("\n%ld  ",linect);
	if (!(makeactive(func))) (void) printf ("*"); /* calls nested too deep */
	else {
		tabstar= 0;
		tabd = tabc;
		for (;tabd >ntabs; tabstar++) tabd = tabd-ntabs;
		for (i = 0 ; i < tabstar ; i++ ) (void) printf ("<");
		(void) printf (" ");
		for (i = 0 ; i < tabd ; i++ ) (void) printf ("\t");
		if (active(func)) (void) printf ("^ %s ^",func->namer); /* recursive call */
		else {
			if (func->dlistp){
				...
				if (!terse || !func->rnameout) {
					...
					(void) printf (" ... [see line %ld]", func->rnameout);
			}
			else (void) printf ("%s [ext]",func->namer); /* library or external call */
		}
		(void) backup();
	}
	return;
