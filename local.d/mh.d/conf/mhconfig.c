...
#ifndef	lint
static char ident[] = "@(#)$Id: mhconfig.c,v 1.2 1995-04-18 16:13:14 vrs Exp $";
#endif	lint
...
static  do_files () {
    ...
    for (pp = files; *pp; pp++) {
	shell ("rm -f %s", *pp);
	shell ("sed -f %s < config/%s > %s", SED, tail (*pp), *pp);
    }
}
...
static  do_make () {
	...
	shell ("rm -f %s", buffer);
	shell ("sed -f %s < %s > %s", SED, pp -> infile, buffer);
    }
}
