...
#define	DIR		FILE
#define opendir(d,m)	fopen(d,m)
#define closedir(dp)	fclose(dp)
#define readdir(dp)	(fread((char *)&_dir,sizeof _dir,1,dp)!=1? NULL : &_dir)
struct direct _dir;
#endif
...
{	register int i, csys;
	...
	{	if(dentp->d_ino == 0)
		...
		{	strcpy(fnam, dentp->d_name);
			...
			/*
			 *	This is for systems that do not
			 *	record the retry time in the file.
			*/
			if (sys[csys].st_retry == 0L)
				sys[csys].st_retry = RETRYTIME;
		}
	}
	closedir(dirp);
}
