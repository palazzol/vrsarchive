...
static char rcsid[]="$Header: /home/Vince/cvs/net/starchart/starchart/starsupp.c,v 1.1 1990-03-30 16:39:42 vrs Exp $";
...
/* stereographic projection */
    ...
    theta = acos(sin_dlcen*Dsinlat + cos_dlcen*Dcoslat*Dcosrac_l);
...
/* gnomonic projection */
	...
	(int)xfs_inv*
      ...
      *xloc = xf_xcen - Dcoslat * Dsinrac_l / denom + 0.5;
      *inregion = ((*xloc >= xf_w_left) && (*xloc <= xf_w_right)
		   && (*yloc <= xf_w_top) && (*yloc >= xf_w_bot));
    } else *inregion = FALSE;
    break;
  case ORTHOGR:
    rac_l = lon - xfs_ra_cen;
	...
	(int)xfs_inv * xfs_scale
      ...
      *xloc = xf_xcen - xfs_scale * Dcoslat * Dsinrac_l;
      *inregion = ((*xloc >= xf_w_left) && (*xloc <= xf_w_right)
		   && (*yloc <= xf_w_top) && (*yloc >= xf_w_bot));
    } else *inregion = FALSE;
    break;
  case RECTANGULAR:
    ...
    *yloc = xf_ycen + (int)xfs_inv * xfs_scale * (lat - xfs_dl_cen);
    *xloc = xf_xcen + xfs_scale * (xfs_ra_cen - lon);
    *inregion = ((*xloc >= xf_w_left) && (*xloc <= xf_w_right)
		 && (*yloc <= xf_w_top) && (*yloc >= xf_w_bot));
    break;
  default:
