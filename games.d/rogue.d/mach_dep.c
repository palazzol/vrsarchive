 ...
 *
 * @(#)mach_dep.c	6.2 (Berkeley) 5/28/84
 */
...
ucount()
{
		...
		while (fread((char *)up, 1, sizeof (*up), utmp) > 0)
				if (buf.ut_name[0] != '\0')
						count++;
		fclose(utmp);
		return count;
...
over:
		...
		if (time((long *)NULL) - sbuf.st_mtime > 10)
		{
		...
		{
				printf("The score file is busy.  Do you want to\n");
				printf("wait so your score can get posted?\n");
				printf("If so, type \"y\"\n");
