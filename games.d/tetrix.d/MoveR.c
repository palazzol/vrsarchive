...
switch (Type) {
	...
	/*  RED PIECES  */
	case R_TYPE   : /*  */
		if (IS_FREE(Column,Row+1) && IS_FREE(Column+2,Row)) {
			PUTCH(Column-1,Row,NO_CHAR);
	...
	case T_TYPE-3 : /*  */
		if (IS_FREE(Column+2,Row) && IS_FREE(Column+2,Row+1)) {
			PUTCH(Column,Row,NO_CHAR);
			PUTCH(Column,Row+1,NO_CHAR);
			PUTCH(Column+2,Row,T_CHAR);
			PUTCH(Column+2,Row+1,T_CHAR);
			Column +=1;
			}
		else goto beepout;
		goto out;
	...
	case V_TYPE-2 : /* checked */
		if (IS_FREE(Column+3,Row)) {
			PUTCH(Column-1,Row,NO_CHAR);
			PUTCH(Column+3,Row,V_CHAR);
			Column +=1;
			}
		else goto beepout;
		goto out;
