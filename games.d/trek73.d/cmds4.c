#ident "@(#) TREK73 $Header: /home/Vince/cvs/games.d/trek73.d/cmds4.c,v 1.1 2002-11-22 04:12:22 Vincent Exp $"
/*
 * $Source: /home/Vince/cvs/games.d/trek73.d/cmds4.c,v $
 *
 * $Header: /home/Vince/cvs/games.d/trek73.d/cmds4.c,v 1.1 2002-11-22 04:12:22 Vincent Exp $
 *
 * $Log: not supported by cvs2svn $
 * Revision 1.3  87/10/13  16:00:59  16:00:59  okamoto (Jeff Okamoto)
 * Fixed calls to Gets
 * 
 * Revision 1.2  87/10/09  14:01:55  14:01:55  okamoto (Jeff Okamoto)
 * Fix bug in self-destruct code.
 * 
 * Revision 1.1  87/10/09  11:02:50  11:02:50  okamoto (Jeff Okamoto)
 * Initial revision
 * 
 */
/*
...
struct ship *sp;
{
	...
	if ((buf1 != NULL) && (buf1[0] != 'n')) {
		...
		if (buf1 != NULL) {
			temp = atoi(buf1);
			if ((temp >= 0) && (temp < MAX_TUBE_SPEED + 1))
				sp->t_lspeed = temp;
		}
		...
		if (buf1 != NULL) {
			temp = atoi(buf1);
			if ((temp >= 0) && (temp < (int)(MAX_TUBE_TIME + 1)))
				sp->t_delay = temp;
		}
		...
		if (buf1 != NULL) {
			temp = atoi(buf1);
			if ((temp >= 0) && (temp < MAX_TUBE_PROX + 1))
				sp->t_prox = temp;
		}
	}
...
struct ship *sp;
{
	...
	if (buf1[0] != 'e' && buf1[0] != 'E') {
		...
		sp->energy = min(sp->energy + phaser_charge, sp->pods);
	for (i=0;i<SHIELDS;i++) 
