...
/*
 *	Stubbs for a printf efficiency hack (start buffering output).
 *	Returns 0 if output was already buffered, else 1.
*/
_stbuf(iop)
FILE *iop;
{ return(0);
...
/*
 *	Undo the work of _stbuf().  _ftbuf(iop, _stbuf(iop)) is a no-op,
 *	since if buffering is in effect, nothing happens, and if not, it
 *	is set then cancelled.  For us _stbuf() returns 0, so a null _ftbuf()
 *	suffices.
*/
_ftbuf(iop, flag)
FILE *iop;
