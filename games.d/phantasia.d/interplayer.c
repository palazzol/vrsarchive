...
/ RETURN VALUE: none
/
/ MODULES CALLED: writerecord(), inputoption(), floor(), wmove(), random(), 
...
HIT:		
	...
	case '3':	/* power blast */
	    ...
	    Player.p_mana -= dtemp;
	    dtemp *= (random() + 0.5) * Player.p_magiclvl * 0.2 + 2.0;
	    mvprintw(Lines++, 0, "You blasted %s !", Enemyname);
	    goto HIT;

	case '4':	/* luckout */
	    if (Luckout || random() > 0.1)
		{
		if (Luckout)
    ...
    else if (random() * Player.p_speed < random() * Other.p_speed)
	/* relinquish control */
	{
	++Player.p_tampered;
	Foestrikes = TRUE;
...
/	floor(), wmove(), waddch(), random(), printw(), altercoordinates(), 
...
long	loc;			/* location in file of other players */
	...
	case T_GRAIL:
	    ...
	    Enrgyvoid.ev_x = ROLL(-1e6, 2e6);
	    Enrgyvoid.ev_y = ROLL(-1e6, 2e6);
...
/	floor(), wmove(), random(), fclose(), fwrite(), sscanf(), strcmp(), 
...
FILE	*fp;				/* for opening gold file */
	...
	if (!Wizard)
	    Player.p_age += N_AGE;		/* penalty for using this stuff */
