...
/ RETURN VALUE: none
/
/ MODULES CALLED: death(), floor(), wmove(), random(), waddstr(), explevel()
/
...
/	sleep(), floor(), wmove(), random(), wclear(), printw(), 
...
bool	dishonest = FALSE;/* set when merchant is dishonest */
	...
	switch(ch)
		...
		if (ch - '0' > size)
			...
			case '3':
				...
				if (random() < 0.02)
				    dishonest = TRUE;
				else if (random() * numitems > Player.p_level / 10.0
				    && numitems != 1)
				    {
				    printw("\nYou blew your mind!\n");
				    Player.p_brains /= 5;
	...
	if (cheat == 1)
		...
		ROLL(Circle * 20.0, 40.0), (random() < 0.5) ? "him" : "her");
...
/	fseek(), execl(), fopen(), floor(), wmove(), random(), wclear(), strcmp(), 
...
/ RETURN VALUE: none
/
/ MODULES CALLED: fabs(), floor(), random(), distance()
/
...
/ RETURN VALUE: none
/
/ MODULES CALLED: death(), floor(), random(), explevel(), movelevel()
/
