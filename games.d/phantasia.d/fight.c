...
/	awardtreasure(), more(), death(), wmove(), setjmp(), random(), printw(), 
...
int	howmany = 1;			/* how many time flocked */
	...
	for (;;)
	    ...
	    if (random() * Curmonster.m_speed > random() * Player.p_speed
		/* monster is faster */
    ...
    if (random() < Curmonster.m_flock / 100.0)
	/* monster flocks */
	{
	more(Lines);
	++howmany;
	...
	&& Curmonster.m_treasuretype > 0
	&& random() > 0.2 + pow(0.4, (double) (howmany / 3 + Circle / 3.0)))
...
/	floor(), wmove(), random(), altercoordinates(), waddstr(), mvprintw(), 
/	wclrtoeol(), wclrtobot()
/
...
int	ch;		/* input */
	...
	case '3':		/* evade */
		...
		|| random() * Player.p_speed * Player.p_brains
		    > random() * Curmonster.m_speed * Curmonster.m_brains)
	...
	case '6':	/* luckout */
		...
		if (Curmonster.m_type == SM_MORGOTH)
		    ...
		    {
		    if ((random() + 0.333) * Player.p_brains
			< (random() + 0.333) * Curmonster.m_brains)
...
/	random(), altercoordinates(), longjmp(), waddstr(), mvprintw(), 
/	getanswer()
/
...
int	ch;			/* input */
	...
	default:
		...
		case SM_JABBERWOCK:
			...
			Enemyname);
		    Whichmonster = 55 + (random() > 0.5) ? 22 : 0;
		    longjmp(Fightenv, 0);
		    /*NOTREACHED*/
    ...
    inflict = random() * Curmonster.m_strength + 0.5;
SPECIALHIT:
...
/	random(), altercoordinates(), longjmp(), infloat(), waddstr(), mvprintw(), 
/	getanswer()
/
...
int	ch;		/* input */
	...
	switch (ch)
		...
		if (Player.p_magiclvl < ML_XPORT)
			...
			< Curmonster.m_experience / 200.0 * random())
			{
		...
		if (Player.p_magiclvl < ML_PARALYZE)
			...
			Curmonster.m_experience / 1000.0 * random())
			{
...
/ RETURN VALUE: none
/
/ MODULES CALLED: truncstring(), fread(), fseek(), floor(), random(), 
/	strcpy()
/
...
struct monster	Othermonster;		/* to find a name for mimics */
	...
	if (Player.p_specialtype < SC_COUNCIL)
	    ...
	    {
	    Curmonster.m_strength *= random() + 0.5;
	    Curmonster.m_brains *= random() + 0.5;
	    Curmonster.m_speed *= random() + 0.5;
	    Curmonster.m_energy *= random() + 0.5;
	    Curmonster.m_experience *= random() + 0.5;
	    ...
	    Curmonster.m_strength = random() * (Player.p_maxenergy + Player.p_shield) / 1.4
		+ random() * (Player.p_maxenergy + Player.p_shield) / 1.5;
...
/	floor(), wmove(), random(), sscanf(), printw(), altercoordinates(), 
...
double	dtemp;				/* for temporary calculations */
	...
	if (ch == 'Y')
	    if (random() < treasuretype / 35.0 + 0.04)
		/* cursed */
		{
	...
	if (ch != 'Y')
	    ...
	    if (random() < 0.08 && Curmonster.m_treasuretype != 4)
		{
