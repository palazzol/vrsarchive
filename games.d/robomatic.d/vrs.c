...
{ 
	...
	if (r == tr && r != atrow) { /* Check collision up */
		cr1 = tr - (d+1)/2 - 1;
		cr2 = tr + (d+1)/2 + 1;
		cc1 = cc2 = (tc + c) / 2;
	...
	if (c == tc && c != atcol) { /* Check collision left */
		cr1 = cr2 = (tr + r) / 2;
		cc1 = tc - (d+1)/2 - 1;
		cc2 = tc + (d+1)/2 + 1;
		if (distance (atrow, atcol, cr1, cc1) < distance (atrow, atcol, cr2, cc2)) { 
...
{ 
	...
	if (envsize < 0) {				/* first time put_env called */
