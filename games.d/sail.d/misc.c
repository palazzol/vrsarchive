...
distance(x,y)
{	 return(abs(x) >= abs(y) ? abs(x) + abs(y)/2 : abs(y) + abs(x)/2);
}
...
#ifndef BERKELEY
/*
 *	Use Unix calls Do these right - the Berkeley kludges are
 *	convenient, but not at all portable.
*/
unsigned alarm_left = 0;		/* Initially none remembered	*/

void
blockalarm()
{ if (alarm_left == 0)
    alarm_left = alarm(0);		/* Get time left, cancle alarm	*/
}
void
unblockalarm()
{ alarm_left = alarm(alarm_left);	/* Reset alarm time		*/
}
#endif
